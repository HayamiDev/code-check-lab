...
export default function CodeViewer({ code, language, className = '' }: CodeViewerProps) {
  ...
  return (
    <div className={`rounded-xl overflow-hidden bg-white/50 dark:bg-[#1e1e1e] border border-slate-200 dark:border-slate-800 shadow-sm relative group flex flex-col ${className}`} role="region" aria-label={`${language}コード`}>
      {/* Code Content */}
      <div className="overflow-auto flex-1 w-full">
        <table className="w-full border-collapse text-xs sm:text-sm font-mono leading-6" role="presentation">
          <tbody>
            {highlightedLines.map((line, index) => (
              <tr key={index} className="hover:bg-slate-100 dark:hover:bg-[#2a2d2e] transition-colors">
                <td className="sticky left-0 select-none text-right pr-4 pl-4 py-0.5 text-slate-400 dark:text-slate-600 bg-slate-50/50 dark:bg-[#1e1e1e] w-[1%] min-w-[3rem] border-r border-slate-200 dark:border-slate-800/50 align-top" aria-hidden="true">
                  {index + 1}
                </td>
